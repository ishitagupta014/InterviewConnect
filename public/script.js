...
function upChat() {
  ...
  setTimeout(function () {
    ...
    else document.getElementById("default_image").style.visibility = "visible";
    if (state_others == -1) {
      document.getElementById("default_image_others").style.visibility =
        "visible";
    } else if (state_others == 0)
      document.getElementById("video-grid2").style.visibility = "visible";
    else
      document.getElementById("default_image_other").style.visibility =
        "visible";
  }, 500);
}
...
//   swal("Want To Add/Change Question", {
//     buttons: {
//       cancel: "No",
//       Yes: true,
//     },
...
socket.on("changeQues", (ques, tabId) => {
  ...
  $("pre").each(function () {
    $(this).html(
      $(this).html().replace(/input/g, "<span class='green'>INPUT</span>")
    );
    $(this).html(
      $(this)
        .html()
        .replace(/output/g, "<span class='red'>OUTPUT</span>")
    );
  });
});
...
socket.on("add_ques_tab_event", function () {
  ...
  $("#ex1").append(
    "<li>" +
      ...
      "'" +
      "" +
      "><span style='color: white;'>Question-" +
      current_idx +
      "</span></a" +
      ">" +
      "</li>"
  ...
  $("#questabs").append(
    '<div  class="tab-pane fade"  id="question' +
      ...
      '<pre id="quesSec' +
      current_idx +
      '" style="max-height: 780px; overflow-y: scroll;">' +
...
socket.on("message-from-others", function (data) {
  var html =
    ...
    '<div class="message others-message"> <strong>' +
    data.user +
    ":</strong> " +
    data.message +
    " </div>" +
...
function sendMessage() {
  ...
  if (message != "") {
    var html =
      ...
      '<div class="message my-message"> <strong>' +
      user +
      ":</strong> " +
      message +
      " </div>" +
...
//meeting controls
if (user === "Interviewer") {
  document.getElementById("meeting_controls").textContent =
    "End Interview and Download Report";
} else {
...
function addVideoStream(video, stream, c) {
  ...
  if (c == 1) videoGrid1.append(video);
  else videoGrid2.append(video);
}
